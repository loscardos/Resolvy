...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.servicePackage.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.service-packages.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bandwidth_mbps_down') ? 'is-invalid' : '' }}" type="number" name="bandwidth_mbps_down" id="bandwidth_mbps_down" value="{{ old('bandwidth_mbps_down', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bandwidth_mbps_up') ? 'is-invalid' : '' }}" type="text" name="bandwidth_mbps_up" id="bandwidth_mbps_up" value="{{ old('bandwidth_mbps_up', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active">
                    ...
                    @foreach(App\Models\ServicePackage::IS_ACTIVE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_active', 'active') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
