...
@section('content')
    ...
    <div class="d-flex justify-content-between mb-4">
        <h3 class="mb-0">{{ trans('global.create') }} {{ trans('cruds.customer.title') }}</h3>
    </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('contact_phone') ? 'is-invalid' : '' }}" type="text" name="contact_phone" id="contact_phone" value="{{ old('contact_phone', '') }}" required>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                                    @foreach(App\Models\Customer::TYPE_SELECT as $key => $label)
                                        <option value="{{ $key }}" {{ old('type', 'individual') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @if($errors->has('type'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('subscription_status') ? 'is-invalid' : '' }}" name="subscription_status" id="subscription_status" required>
                                @foreach(App\Models\Subscription::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('subscription_status', 'active') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
